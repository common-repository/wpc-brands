...
! defined( 'WPCBR_VERSION' ) && define( 'WPCBR_VERSION', '1.3.0' );
! defined( 'WPCBR_LITE' ) && define( 'WPCBR_LITE', __FILE__ );
		...
		if ( ! class_exists( 'WPCleverWpcbr' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcbr {
				...
				function wp_init() {
					...
					// shortcode
					add_shortcode( 'wpcbr', [ $this, 'shortcode_brand' ] );
					add_shortcode( 'wpcbr_banner', [ $this, 'shortcode_brand_banner' ] );
					add_shortcode( 'wpcbr_list', [ $this, 'shortcode_brand_list' ] );
				...
				function admin_menu_content() {
                    ...
                    <div class="wpclever_settings_page wrap">
                        <h1 class="wpclever_settings_page_title"><?php echo esc_html__( 'WPC Brands', 'wpc-brands' ) . ' ' . esc_html( WPCBR_VERSION ); ?></h1>
                        <div class="wpclever_settings_page_desc about-text">
                            <p>
								<?php printf( /* translators: stars */ esc_html__( 'Thank you for using our plugin! If you are satisfied, please reward it a full five-star %s rating.', 'wpc-brands' ), '<span style="color:#ffb900">&#9733;&#9733;&#9733;&#9733;&#9733;</span>' ); ?>
                                <br/>
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcbr&tab=settings' ) ); ?>" class="<?php echo esc_attr( $active_tab === 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'Settings', 'wpc-brands' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcbr&tab=shortcodes' ) ); ?>" class="<?php echo esc_attr( $active_tab === 'shortcodes' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
                        ...
                        <div class="wpclever_settings_page_content">
                                            ...
                                            <td>
												...
												if ( ! empty( $logo_sizes ) ) {
													...
													foreach ( $logo_sizes as $logo_size_name => $logo_size_data ) {
														echo '<option value="' . esc_attr( $logo_size_name ) . '" ' . ( $logo_size_name === $logo_size ? 'selected' : '' ) . '>' . esc_html( $logo_size_name . ( ! empty( $logo_size_data['width'] ) ? ' ' . $logo_size_data['width'] . '&times;' . $logo_size_data['height'] : '' ) . ( $logo_size_data['crop'] ? ' (cropped)' : '' ) ) . '</option>';
													}
							...
							<?php } elseif ( $active_tab === 'shortcodes' ) { ?>
                                <table class="form-table">
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <ul class="wpcbr_shortcode_attrs">
                                                ...
                                                <li><em>type</em> - (optional) text | image | both | full</li>
                                            </ul>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <th scope="row">[wpcbr_list]</th>
                                        <td>
                                            List all brands.

                                            <ul class="wpcbr_shortcode_attrs">
                                                <li><em>type</em> - (optional) image | both | full</li>
                                            </ul>
                                        </td>
                                    </tr>
                                </table>
				...
				function action_links( $links, $file ) {
					...
					if ( $plugin === $file ) {
						$settings = '<a href="' . esc_url( admin_url( 'admin.php?page=wpclever-wpcbr&tab=settings' ) ) . '">' . esc_html__( 'Settings', 'wpc-brands' ) . '</a>';
				...
				function shortcode_brand( $attrs ) {
					ob_start();
					...
					if ( $attrs['product_id'] ) {
						...
						if ( is_array( $brands ) && ! empty( $brands ) ) {
							...
							echo '<div class="' . esc_attr( 'wpcbr-wrap wpcbr-wrap-' . $type ) . '">';

							do_action( 'wpcbr_before_wrap', $attrs, $attrs['context'] );
							...
							if ( 'text' === $type ) {
								...
								do_action( 'wpcbr_before_brands', $attrs, $attrs['context'] );

								foreach ( $brands as $brand ) {
									echo '<div class="' . esc_attr( 'wpcbr-brand wpcbr-brand-' . $brand->term_id ) . '">';
									do_action( 'wpcbr_before_brand', $brand, $attrs['context'] );
									...
									if ( $logo_id && ( $type === 'image' || $type === 'both' || $type === 'full' ) ) {
										...
										do_action( 'wpcbr_before_brand_image', $brand, $attrs['context'] );
										echo '<a href="' . esc_url( get_term_link( $brand->term_id ) ) . '" rel="brand"><img src="' . esc_url( wp_get_attachment_image_url( $logo_id, $logo_size ) ) . '" alt="' . esc_attr( $brand->name ) . '"/></a>';
										do_action( 'wpcbr_after_brand_image', $brand, $attrs['context'] );
										echo '</span>';
									}

									if ( $type === 'both' || $type === 'full' ) {
										echo '<span class="wpcbr-brand-info">';
										do_action( 'wpcbr_before_brand_info', $brand, $attrs['context'] );

										echo '<span class="wpcbr-brand-name">';
										do_action( 'wpcbr_before_brand_name', $brand, $attrs['context'] );
										echo '<a href="' . esc_url( get_term_link( $brand->term_id ) ) . '" rel="brand">' . apply_filters( 'wpcbr_brand_name', $brand->name, $brand ) . '</a>';
										do_action( 'wpcbr_after_brand_name', $brand, $attrs['context'] );
										echo '</span>';

										if ( ! empty( $brand->description ) && $type === 'full' ) {
											echo '<span class="wpcbr-brand-description">';
											do_action( 'wpcbr_before_brand_description', $brand, $attrs['context'] );
											echo apply_filters( 'wpcbr_brand_description', $brand->description, $brand );
											do_action( 'wpcbr_after_brand_description', $brand, $attrs['context'] );
											echo '</span>';
										}

										do_action( 'wpcbr_after_brand_info', $brand, $attrs['context'] );
										echo '</span>';
									}

									do_action( 'wpcbr_after_brand', $brand, $attrs['context'] );
									echo '</div>';
								}

								do_action( 'wpcbr_after_brands', $attrs, $attrs['context'] );
				...
				function shortcode_brand_banner( $attrs ) {
					ob_start();
				...
				function shortcode_brand_list( $attrs ) {
					ob_start();
					...
					$type    = $attrs['type'] ?? 'full';
					$context = $attrs['context'] ?? 'list';
					$brands  = get_terms( apply_filters( 'wpcbr_brand_list_args', [
						'taxonomy'   => 'wpc-brand',
						'hide_empty' => false,
					] ) );

					if ( ! empty( $brands ) ) {
						echo '<div class="' . esc_attr( 'wpcbr-list wpcbr-list-' . $type ) . '">';
						echo '<div class="wpcbr-brands">';

						do_action( 'wpcbr_before_brands', $attrs, $context );

						foreach ( $brands as $brand ) {
							echo '<div class="' . esc_attr( 'wpcbr-brand wpcbr-brand-' . $brand->term_id ) . '">';
							do_action( 'wpcbr_before_brand', $brand, $context );
							...
							if ( $logo_id && ( $type === 'image' || $type === 'both' || $type === 'full' ) ) {
								$logo_size = self::get_setting( 'logo_size', 'wpcbr-logo' );
								echo '<span class="wpcbr-brand-image">';
								do_action( 'wpcbr_before_brand_image', $brand, $context );
								echo '<a href="' . esc_url( get_term_link( $brand->term_id ) ) . '" rel="brand"><img src="' . esc_url( wp_get_attachment_image_url( $logo_id, $logo_size ) ) . '" alt="' . esc_attr( $brand->name ) . '"/></a>';
								do_action( 'wpcbr_after_brand_image', $brand, $context );
								echo '</span>';
							}

							if ( $type === 'both' || $type === 'full' ) {
								echo '<span class="wpcbr-brand-info">';
								do_action( 'wpcbr_before_brand_info', $brand, $context );

								echo '<span class="wpcbr-brand-name">';
								do_action( 'wpcbr_before_brand_name', $brand, $context );
								echo '<a href="' . esc_url( get_term_link( $brand->term_id ) ) . '" rel="brand">' . apply_filters( 'wpcbr_brand_name', $brand->name, $brand ) . '</a>';
								do_action( 'wpcbr_after_brand_name', $brand, $context );
								echo '</span>';

								if ( ! empty( $brand->description ) && $type === 'full' ) {
									echo '<span class="wpcbr-brand-description">';
									do_action( 'wpcbr_before_brand_description', $brand, $context );
									echo apply_filters( 'wpcbr_brand_description', $brand->description, $brand );
									do_action( 'wpcbr_after_brand_description', $brand, $context );
									echo '</span>';
								}

								do_action( 'wpcbr_after_brand_info', $brand, $context );
								echo '</span>';
							}

							do_action( 'wpcbr_after_brand', $brand, $context );
							echo '</div>';
						}

						do_action( 'wpcbr_after_brands', $attrs, $context );

						echo '</div>';
						echo '</div><!-- /.wpcbr-list -->';
					}

					return apply_filters( 'wpcbr_list_shortcode', ob_get_clean(), $attrs );
				}
